...
class UserAppliesController < ApplicationController
  ...
  def create
    if UserApply.safe_add(:user => current_user,
      ...
      redirect_to dashboards_path, notice: '组织申请成功，审核中'
    else
      redirect_to dashboards_path, alert: '组织申请失败，你可能已申请过或者正在申请其它企业'
    end
  end
  ...
  def update
    ...
    @apply.pass
    redirect_to user_applies_path(:organ_id => @apply.actor.organ.root), notice: '成功通过组织申请'
  end
